...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [_F1] = LAYOUT_all(
        ...
        KC_RSFT, _______, _______, _______, _______, KC_VRSN, _______, NK_TOGG, _______, _______, _______, _______, KC_LSFT,      _______,     _______, _______, _______, _______,
    ...
    [_F3] = LAYOUT_all(
        ...
        _______, _______, QK_BOOT, EE_CLR,  _______, _______, _______, _______, _______, _______, _______, _______, _______,      _______,     _______, _______, _______, _______,
...
bool is_mac_num_lock;
bool is_win_num_lock;

void keyboard_pre_init_user(void) {
  setPinOutput(ALT_LED_NUM_LOCK_PIN);
  is_mac_num_lock = false;
  is_win_num_lock = false;
}

void __led_update_ports(void) {
    if (is_mac_num_lock || is_win_num_lock) {
        writePin(ALT_LED_NUM_LOCK_PIN, LED_PIN_ON_STATE);
    } else {
        writePin(ALT_LED_NUM_LOCK_PIN, !LED_PIN_ON_STATE);
    }
}
...
layer_state_t layer_state_set_user(layer_state_t state) {
    is_mac_num_lock = get_highest_layer(state) == _NL;
    __led_update_ports();
    return state;
}

bool led_update_user(led_t led_state) {
    is_win_num_lock = led_state.num_lock;
    __led_update_ports();
    return true;
}
